 ...
 * 冪等保證：providerTxnId 已非 PENDING 時直接返回。
 */
export async function applyPaymentOutcome(
  ...
  const retryOrder = await prisma.$transaction(async (tx) => {
    ...
    if (existing) {
      // 已有 PENDING Payment → 更新狀態
      await tx.payment.updateMany({
        where: { providerTxnId: txnId },
        data: { status: paymentStatus },
      })
    } else {
      // 無既有 Payment（webhook 先到）→ 建立記錄
  ...
  // Step 3: tx commit 後才 charge，避免孤兒扣款（同 subscriptionService / billingCron）
  if (retryOrder) {
