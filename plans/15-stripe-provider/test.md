...
- [ ] 2. StripeProvider.charge 續扣 off-session 成功 → 同步回 SUCCESS(無 clientSecret)
- [ ] 3. StripeProvider.charge 續扣 off-session 失敗(StripeCardError)→ 同步回 FAILED
- [ ] 4. 續扣同時帶 customer + payment_method 呼叫 Stripe
- [ ] 5. ChargeResult:clientSecret optional、status 可為 PENDING|SUCCESS|FAILED(Mock 仍 PENDING)
- [ ] 6. /webhooks/stripe 驗簽失敗回 401
- [ ] 7. /webhooks/stripe succeeded → 三表同 tx 更新 + 存 providerPaymentMethodId
- [ ] 8. /webhooks/stripe 重送(同 providerTxnId)冪等回 200,不重複更新
- [ ] 9. 續扣 FAILED → 走 dunning(沿用 #5,同步路徑,不等 webhook)
