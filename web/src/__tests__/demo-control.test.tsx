...
beforeEach(() => {
  mockUseConfig.mockReturnValue({ demoMode: false, provider: 'mock' })
...
describe('19-demo-control 13. provider=stripe → 無 force-fail / replay 鈕', () => {
  it('force-fail 和 replay 鈕不渲染；顯示測試卡說明', async () => {
    mockUseConfig.mockReturnValue({ demoMode: true, provider: 'stripe' })
...
describe('19-demo-control 13b. provider=mock → 顯示 force-fail / replay 鈕', () => {
  it('兩個 Mock 專屬按鈕都可見', () => {
    mockUseConfig.mockReturnValue({ demoMode: true, provider: 'mock' })
    render(<DemoControlPanel />)
